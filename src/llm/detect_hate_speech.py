...
class LLMDetectHateSpeechResult(BaseModel):
    is_hate_speech: bool
    target_of_hate: list[str]
    reasoning: str


def llm_detect_hate_speech(llm: LLMService, text: str) -> LLMDetectHateSpeechResult:
    return llm.run_parse_model(
        prompt=HATE_SPEECH_DETECTION_PROMPT,
        prompt_inputs={INPUT_TEXT_KEY: text},
        output_model=LLMDetectHateSpeechResult,
    )
